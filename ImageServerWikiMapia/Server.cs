...
namespace Manifold.ImageServer.WikiMapia
{
    public abstract class ServerWikiMapia : IServer
	{
		// Server data
        ...
        // WikiMapia URL
        // http://[HOST]/?x=[X]&y=[Y]&zoom=[ZOOM]
        // [HOST] = i[NUM].wikimapia.org
        // [NUM] = [X]%4 + ([Y]%4)*4

		// Constructor
		public ServerWikiMapia()
		{
            m_strName = "";
            m_strDefaultUrl = "";
            m_strUrl = "";
		...
		public Boolean DownloadTile(Int32 _x, Int32 _y, Int32 _scale, String _filename)
		{
            ...
            {
                int num = _x%4 + (_y%4)*4;
                strRequest = strRequest.Replace("*", Convert.ToString(num, CultureInfo.InvariantCulture));
            }

			nIndex = strRequest.IndexOf("?");
            if (nIndex == -1)
                strRequest += "?";
            ...
            strRequest += "&zoom=" + Convert.ToString(m_nScaleHi - _scale, CultureInfo.InvariantCulture);
		...
		public String ScaleNames
		{
            get
            {
                return "0.03 m,0.06 m,0.13 m,0.25 m,0.5 m,1 m,2 m,5 m,10 m,20 m,40 m,80 m,160 m,320 m,640 m,1.3 km,2.5 km,5 km,10 km,20 km,40 km,80 km,160 km";
            }
		}
    ...
    public class ServerWikiMapiaStreetMap : ServerWikiMapia
    {
        public ServerWikiMapiaStreetMap()
            : base()
        {
            m_strName = "WikiMapia Street Map Image";
            m_strDefaultUrl = "http://i*.wikimapia.org/";
			m_strUrl = m_strDefaultUrl;
			m_strImageType = ".png";
        }
    }

    public class ServerWikiMapiaStreetMapTransparent : ServerWikiMapia
    {
        public ServerWikiMapiaStreetMapTransparent()
            : base()
        {
            m_strName = "WikiMapia Street Map Image (Transparent)";
            m_strDefaultUrl = "http://i*.wikimapia.org/?type=hybrid";
            m_strUrl = m_strDefaultUrl;
            m_strImageType = ".png";
        }
    }
